...
def test_image_objects():
    ...
    images = list( page.get_objects(filter=[pdfium.FPDF_PAGEOBJ_IMAGE]) )
    assert len(images) == 3
...
def test_extract_image(render):
    ...
    all_images = list( page.get_objects(filter=[pdfium.FPDF_PAGEOBJ_IMAGE]) )
    image = all_images[0]
    ...
    if render:
        assert bitmap.format == pdfium.FPDFBitmap_BGRA
        ...
        assert bitmap.format == pdfium.FPDFBitmap_Gray
...
def test_remove_image():
    ...
    images = list( page_1.get_objects(filter=[pdfium.FPDF_PAGEOBJ_IMAGE]) )
    assert len(images) == 3
